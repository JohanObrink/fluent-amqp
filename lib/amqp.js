...
function createContext (options) {
  ...
  if (!contexts[key]) {
    contexts[key] = getConnection(options)
      .then(connection => {
        logger.debug('connection.createChannel()')
        connection.once('close', () => {
          contexts[key] = null
        })
        return connection.createChannel()
          .then(channel => {
            channel.on('error', (err) => logger.warn(err))
...
function deleteExchange () {
  if (!this.ctx || !this.ctx.channel || !this.ctx.exchangeName) {
    ...
    return this.ctx.channel.deleteExchange(this.ctx.exchangeName).catch(() => ({}))
  }
}
...
function deleteQueue () {
  if (!this.ctx || !this.ctx.channel || !this.ctx.queue) {
    ...
    return this.ctx.channel.deleteQueue(this.ctx.queue).catch(() => ({}))
  }
}
    ...
    .then(ctx => {
      ...
      ctx.channel.consume(ctx.queueName, (msg) => {
        if (!msg && stream.next) {
          ...
          const ack = (msg, allUpTo) => createContext(options)
            .then(ctx => ctx.channel.ack(msg, allUpTo))
            .catch(err => {
              if (err.message === 'Channel closing' || err.message === 'Channel closed') {
                logger.debug('Retry ack')
                return ack(msg, allUpTo)
              } else {
                return Promise.reject(err)
              }
            })
          const nack = (msg, allUpTo, requeue) => createContext(options)
            .then(ctx => ctx.channel.nack(msg, allUpTo, requeue))
            .catch(err => {
              if (err.message === 'Channel closing' || err.message === 'Channel closed') {
                logger.debug('Retry nack')
                return nack(msg, allUpTo, requeue)
              } else {
                return Promise.reject(err)
              }
            })
          stream.push(null, Object.assign(msg, {
            ...
            ack: (allUpTo) => ack(msg, allUpTo),
            nack: (allUpTo, requeue) => nack(msg, allUpTo, requeue)
          }))
        }
